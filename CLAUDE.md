...
- `internal/spells/` — `Spell` (data-driven: `ManaCost`, `Phases`, `TargetType`, composable `Effect`s with `Params`), the hardcoded `Catalog` (11 spells), `BuildDeck` (40-card recipe), and `PlayerState` (hand/deck/discard/mana). Effect kinds: `noop`, `destroy_piece`, `freeze_piece`, `shield_piece`, `draw_card`, `gain_mana`, `move_piece`. `TargetType.TargetCount()` returns the required target count (0 for `none`, 2 for `piece_move`, else 1). Mana constants: `InitialMana=1`, `MaxManaCap=10`, `StartingHand=4`, `DeckSize=40`.
