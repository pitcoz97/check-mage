...
export function PromotionDialog({ color, onChoose, onCancel }: { color: Color; onChoose(letter: string): void; onCancel(): void }) {
  ...
  return (
    <div
      ...
      onClick={onCancel}
    >
      <div className="flex gap-2 rounded-10 border border-subtle bg-panel p-3" onClick={(event) => event.stopPropagation()}>
        {PROMOTION_CHOICES.map((choice) => (
          <button
            key={choice.letter}
            type="button"
            aria-label={t(`board.piece.${choice.kind}`)}
            className="size-14 rounded-8 bg-elevated p-1 hover:bg-app focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent"
