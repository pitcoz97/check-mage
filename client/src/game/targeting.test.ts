...
function collecting(spell: Spell): TargetingState {
  const outcome = beginTargeting(card, spell);
...
describe('targeting', () => {
  it('una magia senza bersagli parte subito', () => {
    expect(beginTargeting(card, testSpell({ targets: [] }))).toEqual({ kind: 'cast', card, targets: [] });
  });
  ...
  it('un tipo di bersaglio sconosciuto non apre il targeting', () => {
    expect(beginTargeting(card, testSpell({ targets: [targetSpec('constellation')] }))).toEqual({
      kind: 'refused',
